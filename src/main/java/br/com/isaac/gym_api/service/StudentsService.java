...
public class StudentsService {
    ...
    public void create(CreateStudentRequestDTO newStudent) {
        studentsRepository.save(StudentsEntity.builder()
    ...
    public List<StudentsResponseDTO> findAll() {
        return studentsRepository.findAll().stream().map(StudentsResponseDTO::fromEntity).toList();
    }

    public List<PhysicalAssessmentResponseDTO> findAllPhysicalAssessment(UUID id) {
        return physicalAssessmentRepository.findAllByStudentId(id).stream().map(PhysicalAssessmentResponseDTO::fromEntity).toList();
    }
}
